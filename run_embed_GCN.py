...
class Device_encoder(torch.nn.Module):
    ...
    def get_inputs2(self, inputs):
        one, two, three, four, \
            firstIdx, secondIdx, thirdIdx, fourthIdx, fifthIdx, sixthIdx, seventhIdx, eighthIdx, nineIdx = inputs
        op_idx = torch.vstack([firstIdx, secondIdx, thirdIdx, fourthIdx, fifthIdx, sixthIdx, seventhIdx, eighthIdx, nineIdx])
        op_idx = op_idx.to(torch.long).T
        # 获得计算节点信息
        platformIdx = one
        deviceIdx = two
        device_name_Idx = three
        precisionIdx = four
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='embed_gcn')  #
