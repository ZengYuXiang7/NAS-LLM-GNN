...
def get_train_valid_test_dataset(tensor, args):
    ...
    # 分割数据集为训练集和临时测试集（可能还包括验证集）
    # 通过args.density来决定训练集大小
    X_train, X_temp, Y_train, Y_temp = train_test_split(X, Y, test_size=1-args.density)

    # 如果需要验证集，进一步分割临时测试集为验证集和测试集
    test_size = 0.05 / (1 - args.density)  # 确保测试集是剩余部分的5%
    X_valid, X_test, Y_valid, Y_test = train_test_split(X_temp, Y_temp, test_size=1-test_size)

    # 将数据重新组合为训练集、验证集（如果有）和测试集的tensor
